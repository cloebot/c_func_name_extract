...
def get_ast(source_folder, input_file, temp_file):
    cmd = "clang -Xclang -ast-dump -I " + source_folder + " " + source_folder + input_file + " > " + temp_file
    input_file_name = re.findall(r"([a-zA-Z0-9_]*.cpp|[a-zA-Z0-9_]*.h)", input_file)
...
#        m = re.findall(r"(\<.*>\s[a-zA-Z0-9_]*)", func_str)
        m = re.findall(r"(\<.*\,\sline.*\>\sline:[0-9]*:[0-9]*\s[a-zA-Z0-9_]*)", func_str)
        if m:
...
#            temp_line = [input_file, func_name, start_line, end_line, end_line - start_line + 1]
            result_list.append([input_file_name[-1], func_name, start_line, end_line, end_line - start_line + 1])

    return result_list

#~/firmware/code_clone/bellon_c/postgresql/src/backend/nodes/equalfuncs.c > clang.log

def source_to_ast(source_folder, input_file, output_file):

    func_list = get_ast(source_folder, input_file, output_file)
    write_to_csv(func_list, output_file)

if __name__ == '__main__':

    if len(sys.argv) != 4:
        print "python func_extract_clang.py <source_folder> <input_file> <output_file>"
        exit(-1)
    source_to_ast(sys.argv[1], sys.argv[2], sys.argv[3])
